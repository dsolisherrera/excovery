...
def get_packet_list(file, timediff):
    '''
    ...
    while done==False:
        try:
            ...
            #			ipaddr=pcap.ntoa(struct.unpack('i',s[12:16])[0])
...
def run_dir_importer(exp_dir, run_identifier, db_run_id, node, c,olsr_fmt):
	...
	# parse packets
	if node['abstract_id']!="": 
		cap = "%s/nodes/%s/capture/%s.pcap" %(exp_dir, run_identifier,node['real_id'])
		packet_list = get_packet_list(cap, time_diff)
		#print len(packet_list)
		#print packet_list
		for packet in packet_list:
			c.execute("INSERT INTO Packets (RunID,NodeID,CommonTime, SrcNodeId, Data) VALUES (?,?,?,?,?)", [db_run_id,node['real_id'],packet['CommonTime'],packet['SrcNodeID'],sqlite3.Binary(packet['Data'])])	 
